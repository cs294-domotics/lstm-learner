...
DESIRED_TYPES = ['M', 'D', 'L']
LIGHT_TYPE = 'L'
SAVE_FOLDER = "stateful_data/hour/"
...
def generate_and_save_fixed_interval_data(event_timestamps, stateful_event_data, interval_secs, device_indices):
    column_labels = "Timestamp " + "Hour " + " ".join(device_indices.keys()) + "\n"
    ...
    for interval_timestamp in interval_timestamps:
        while interval_timestamp > next_event_timestamp:
            ...
            current_state = stateful_event_data[curr]
            next_event_timestamp = event_timestamps[curr+1]
        line_list = [interval_timestamp] + [encode_hour(interval_timestamp)] + current_state
...
def encode_hour(timestamp):
    date = datetime.datetime.fromtimestamp(timestamp)
    #if date.hour > 6 and date.hour < 18:
    #    time_of_day = 1 #day
    return round(date.hour/24.0, 2)
