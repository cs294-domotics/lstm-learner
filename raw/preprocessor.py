...
def main():
    ...
    input_types = ['M', 'D'] #only motion and door sensors will be used in feature vector
    input_vectors = []
    input_vector, device_indices = initialize_vector(device_type_counts, device_type_buckets, input_types)
    for t in range(num_events):
        if is_well_formed(data[t]) and get_device_type(get_device(data[t])) in input_types:
            ...
            input_vector[device_indices[device]] = value
...
def initialize_vector(device_type_counts, device_type_buckets, input_types):
